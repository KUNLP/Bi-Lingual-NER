...
class CoNLLReader(Dataset):
    ...
    def read_data(self, data):
        ...
        for fields, metadata in get_ner_reader(data=data):
            ...
            if len(fields)<1:
                ...
                continue
            try:
                language = metadata.split('\t')[-1]
            except:
                print(data)
                continue
            sentence_str, tokens_sub_rep, token_masks_rep, coded_ner_, gold_spans_, mask = self.parse_line_for_ner(fields=fields,language=language)
            #print(f'sentence_str : {sentence_str}\ntokens_sub_rep : {tokens_sub_rep}\ntoken_masks_rep : {token_masks_rep}\ncoded_ner_ : {coded_ner_}\ngold_spans_ : {gold_spans_}\nmask : {mask}')
    ...
    def parse_line_for_ner(self, fields,language):
        tokens_, ner_tags = fields[0], fields[-1]
        sentence_str, tokens_sub_rep, ner_tags_rep, token_masks_rep, mask = self.parse_tokens_for_ner(tokens_, ner_tags,language)
    ...
    def parse_tokens_for_ner(self, tokens_, ner_tags,language):
        # 언어 코드 추가
        self.tokenizer.add_special_tokens({"additional_special_tokens": ['<ko>', '<en>', '<mixed>']})

        tokens_.insert(0,language)
        ner_tags.insert(0,'O')
